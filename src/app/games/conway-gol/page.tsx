  ...
  const ConwayGame = () => {
    ...
    const fadeMapRef = useRef<Map<number, number>>(new Map()); // (row, col) => time remaining
    ...
    const renderLoop = (timeStamp: number) => {
      ...
      if (elapsed >= speedRef.current || lastTimeRef.current === 0) {
        ...
        if (changedCellsArray.length > 0) {
          ...
          if ((changedCellsArray.length/2) > (width * height * 0.5)) {
            drawGrid();
            drawCells();
          } else {
            console.log("Redrawing only the changed cells");
      ...
      for (let row = 0; row < height; row++) {
        for (let col = 0; col < width; col++) {
          ...
          if (cells[idx] !== wasm.Cell.Alive) {
                const value = fadeMapRef.current.get(idx);
                if (value !== undefined && value > 0) {
                  const opa = value / 100;
                  ctx.fillStyle = `rgb(232, 142, 187, ${opa})`;  
                  ctx.fillRect(col * CELL_SIZE, row * CELL_SIZE, CELL_SIZE, CELL_SIZE);
                 
                  const newValue = Math.max(0, value - 10);
                  if (newValue === 0) {
                    fadeMapRef.current.delete(idx);
                  }else {
                    fadeMapRef.current.set(idx, newValue);
                  }
            ...
            fadeMapRef.current.set(idx, 100); 
    ...
    const handleMouseDown = (event: React.MouseEvent<HTMLCanvasElement>) => {
      if (isPlaying) {
        showCursorMessage("Please pause the game first", event.clientX, event.clientY);
        return;} // don't allow dragging while playing, or pause first
      setIsDragging(true);
  ...
  const showCursorMessage = (message: string, x:number, y:number) => {
    const messageElement = document.createElement('div');
    messageElement.textContent = message;
    messageElement.style.position = 'fixed';
    messageElement.style.top = `${y+10}px`;
    messageElement.style.left = `${x+10}px`;
