    ...
    impl MazeState {
       ...
       pub fn get_maze_info(&self) ->  Result<JsValue, JsValue> {
          let target = self.target;
          let computer_player = self.computer_player;
          Ok(JsValue::from_str(&format!("target:{:?} & computer_player:{:?}", target, computer_player)))
       }
